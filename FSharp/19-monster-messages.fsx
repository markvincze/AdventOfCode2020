...
let doesMatch rule str =
    let rec doesMatch rule (str : string) index =
        match rule with
        | Char char -> match str.[index] with
                       | c when c = char -> true, (index + 1)
                       | _ -> false, index
        | Choice (r1, r2) -> let m1, i1 = doesMatch r1 str index
                             let m2, i2 = doesMatch r2 str index
                             if m1 then m1, i1
                             else if m2 then m2, i2
                             else false, index
        | Sequence rules -> List.fold
                                (fun (m, i) r -> if m then doesMatch r str i
                                                 else m, i)
                                (true, index)
                                rules
    let (m, i) = doesMatch rule str 0
    m && i = (str.Length)
...
let rec parse (rulesInput : string array) ruleCache =
    let nextLine =
        rulesInput
    ...
    | Some line ->
        ...
        parse rulesInput (Map.add ruleId rule ruleCache)

let rulesInput = input
                 |> Array.takeWhile (fun l -> l <> "")

let rootRule = parse rulesInput Map.empty<int, Rule>
